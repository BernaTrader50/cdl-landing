...
const PRODUCTS = [
  ...
  { brand:"Zendure", model:"SuperBase Pro 2000", price:1699, wh:1850, surge:4000, solar:800, ups:true, expandable:false, weight:46, warranty:5, scores:{home_backup:7,rv:6,camping:4,off_grid:7,apartment:5,medical:8,value:5}, notes:"UPS 13ms. AmpUp to 3000W." },
...
const SCENARIO_SCORE_MAP: Record<string, string> = {
...
function computePicks(scenario: string, appliance: string, days: string, budget: number) {
  const app = APPLIANCE_DATA[appliance];
  ...
  const surgeOk = PRODUCTS.filter(p => p.surge >= requiredSurge && (!app.needsUps || p.ups));
  const inBudget = budget > 0 ? surgeOk.filter(p => p.price <= budget) : surgeOk;
  const scored = [...inBudget].sort((a, b) => b.scores[scoreKey] - a.scores[scoreKey]);

  const bestMatch = scored[0];
  const bestValue = [...inBudget].filter(p => p !== bestMatch).sort((a, b) => (b.wh / b.price) - (a.wh / a.price))[0];
  const premiumPick = [...surgeOk].filter(p => p !== bestMatch && p !== bestValue).sort((a, b) => b.scores[scoreKey] - a.scores[scoreKey])[0];
  ...
  const picks: Pick[] = raw
    ...
    .map(({ p, label }) => {
      let reason = p.notes;
      if (label === "Best Value") reason = `Best capacity-per-dollar: ${(p.wh / p.price).toFixed(2)} Wh/$. ${p.notes}`;
      if (label === "Premium Pick") reason = `Highest ${scoreKey.replace("_", " ")} score (${p.scores[scoreKey]}/10). ${p.notes}`;
      return { product: p, label, reason, limitation: getLimitation(p, appliance) };
    });

  return { picks, requiredWh, requiredSurge, outOfBudget, scoreKey };
}

function FieldIcon({ kind }: { kind: "home" | "fridge" | "dollar" | "calendar" }) {
  const common = { viewBox:"0 0 24 24", className:"h-[18px] w-[18px] text-neutral-500", fill:"none" as const, stroke:"currentColor", strokeWidth:1.6, strokeLinecap:"round" as const, strokeLinejoin:"round" as const };
...
export function SolarCalculator() {
  ...
  const LABEL_COLORS: Record<string, string> = { "Best Match":"#2563EB", "Best Value":"#10B981", "Premium Pick":"#8B5CF6" };
    ...
    <form onSubmit={onSubmit} className="rounded-[16px] bg-white p-7 md:p-9" style={{ boxShadow:"0 4px 24px rgba(0,0,0,0.08)" }}>
      <div className="grid grid-cols-1 gap-x-5 gap-y-6 sm:grid-cols-2">
        ...
        <Field label="BUDGET (USD)" icon="dollar">
          <input type="number" min={200} max={15000} value={budget} onChange={e => setBudget(e.target.value)} className={selectCls} />
        </Field>
      ...
      <button type="button" onClick={onSubmit} className="group mt-7 inline-flex w-full items-center justify-center gap-2 rounded-[12px] bg-neutral-950 px-5 py-4 text-[14.5px] font-medium text-white transition-all duration-200 hover:-translate-y-[2px] hover:opacity-[0.85]">
      ...
      {result && (
        <div className="mt-7">
          {result.outOfBudget && <p className="mb-3 text-[12.5px] text-neutral-500">No exact matches within budget — showing closest options:</p>}
          <div className="grid grid-cols-1 gap-3 md:grid-cols-3">
            {result.picks.map((p, i) => {
              const accent = LABEL_COLORS[p.label] || "#2563EB";
              const score = p.product.scores[result.scoreKey];
              return (
                <div key={p.product.model + i} className="rounded-[12px] border bg-white p-4 opacity-0 animate-fade-in" style={{ borderColor:"#E2E2E2", borderTop:`3px solid ${accent}`, animationDelay:`${i*100}ms`, animationFillMode:"forwards" }}>
                  <p className="font-mono text-[10.5px] font-medium uppercase tracking-[0.14em]" style={{ color:accent }}>{p.label}</p>
                  <h3 className="mt-1 text-[14px] font-semibold leading-tight text-neutral-950">{p.product.brand} {p.product.model}</h3>
                  <div className="mt-3 grid grid-cols-2 gap-y-1 text-[12px] tabular-nums text-neutral-700">
                    <span className="text-neutral-500">Capacity</span><span className="text-right">{p.product.wh.toLocaleString()} Wh</span>
                    <span className="text-neutral-500">Surge</span><span className="text-right">{p.product.surge.toLocaleString()} W</span>
                    <span className="text-neutral-500">UPS</span><span className="text-right">{p.product.ups ? "✓ Yes" : "✗ No"}</span>
                    <span className="text-neutral-500">Price</span><span className="text-right font-semibold" style={{ color:accent }}>${p.product.price.toLocaleString()}</span>
                  </div>
                  <div className="mt-3 pt-3 border-t" style={{ borderColor:"#F0F0F0" }}>
                    <p className="text-[10px] font-mono uppercase tracking-wider text-neutral-400 mb-0.5">{result.scoreKey.replace("_"," ")} score</p>
                    <ScoreBar score={score} />
                  </div>
                  <p className="mt-3 text-[11.5px] leading-[1.55] text-neutral-600">{p.reason}</p>
                  <div className="mt-2.5 rounded-[6px] bg-neutral-50 px-2.5 py-2 text-[11px] text-neutral-500">⚠️ {p.limitation}</div>
                  <a href={p.product.brand === "EcoFlow" ? "https://us.ecoflow.com" : p.product.brand === "Jackery" ? "https://www.jackery.com" : p.product.brand === "Bluetti" ? "https://www.bluettipower.com" : "https://www.ankersolix.com"} target="_blank" rel="noopener noreferrer sponsored" className="mt-3 block rounded-[8px] py-2.5 text-center text-[12.5px] font-medium text-white transition-opacity hover:opacity-80" style={{ background:accent }}>
                    Check current price →
                  </a>
                </div>
              );
          ...
          <p className="mt-4 text-[11px] text-neutral-400">{PRODUCTS.length} products analyzed · Required: {result.requiredWh.toLocaleString()} Wh · Surge: {result.requiredSurge.toLocaleString()} W · We earn a commission if you purchase — this doesn't affect our recommendations.</p>
        </div>
      )}
    </form>
  );
