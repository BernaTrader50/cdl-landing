...
async function loadData(slug: string) {
  ...
  if (post.categories?.length) {
    ...
    const [catRes, relRes] = await Promise.all([
      ...
      fetch(
        `${API_BASE}/posts?categories=${catId}&exclude=${post.id}&per_page=3&_fields=id,slug,title,excerpt,date,categories`,
      ),
    ]);
    ...
    <div className="min-h-screen bg-white">
      ...
      <main className="px-6 pb-24 pt-32">
        <div className="mx-auto max-w-3xl text-center">
          ...
          <Link
            to="/blog"
            className="mt-6 inline-block rounded-md bg-neutral-900 px-4 py-2 text-[13px] font-medium text-white hover:bg-[#2563EB]"
          >
            ← Back to Research
          </Link>
        </div>
    ...
    <div className="min-h-screen bg-white text-neutral-950">
      ...
      <main className="px-6 pb-24 pt-32">
        <article className="mx-auto max-w-[720px]">
          <Link
            to="/blog"
            className="inline-flex items-center gap-1 text-[13px] text-neutral-600 transition-colors hover:text-[#2563EB]"
          >
            ← Back to Research
          </Link>
          ...
          <div className="mt-6 flex flex-wrap items-center gap-2">
            {post.categories.map((id) => {
              ...
              return (
                <span
                  key={id}
                  className="rounded bg-[#EFF6FF] px-2 py-0.5 text-[11px] font-medium text-[#2563EB]"
                >
                  {cat.name}
                </span>
              );
            ...
            <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-3">
              {related.map((p) => {
                ...
                return (
                  ...
                  >
                    {cat && (
                      <span className="self-start rounded bg-[#EFF6FF] px-2 py-0.5 text-[11px] font-medium text-[#2563EB]">
                        {cat.name}
                      </span>
                    )}
                    <h3
                      className="mt-3 text-[16px] font-semibold leading-[1.3] tracking-[-0.01em] group-hover:text-[#2563EB]"
      ...
      <style
        dangerouslySetInnerHTML={{
          __html: `
            ...
            .wp-content blockquote { border-left: 3px solid #2563EB; padding: 0.2em 0 0.2em 1em; margin: 1.5em 0; color: #525252; font-style: italic; }
            ...
            .wp-content pre { background: #f5f5f5; padding: 1em; border-radius: 8px; overflow-x: auto; }
          `,
        }}
      />
    </div>
