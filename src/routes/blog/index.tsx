...
export const Route = createFileRoute("/blog/")({
  head: () => ({
    meta: [
      { title: "Solar Lab — Research & Guides" },
      {
        name: "description",
        content:
          "Research, technical guides, comparisons, and real-world use cases for solar generators and off-grid power.",
      },
    ],
  }),
  component: BlogIndex,
    ...
    (async () => {
      ...
      try {
        const catRes = await fetch(
          `${API_BASE}/categories?per_page=20&_fields=id,name,slug,count`,
        );
        if (catRes.ok) {
      ...
      <main className="px-6 pb-24 pt-32">
        <div className="mx-auto max-w-6xl">
          ...
          <h1 className="mt-3 text-[40px] font-semibold leading-[1.15] tracking-[-0.025em]">
            Solar Lab — Research & Guides
          </h1>
          <p className="mt-4 max-w-2xl text-[16px] leading-relaxed text-neutral-600">
            In-depth analysis, real-world testing, and decision frameworks for solar generators
            and off-grid power.
          </p>
          ...
          <div className="mt-8">
            {error && !loading ? (
              <div className="rounded-lg border border-[#e8e8e8] bg-white p-8 text-center text-[14px] text-neutral-600">
                ...
                <div className="mt-3">
                  <button
                    onClick={() => handleFilter(activeSlug)}
                    className="rounded-md bg-neutral-900 px-3 py-1.5 text-[13px] font-medium text-white hover:bg-[#2563EB]"
                  >
                    Try again
                  </button>
                </div>
                  ...
                  {posts.map((p) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <h2
                          className="mt-3 text-[18px] font-semibold leading-[1.3] tracking-[-0.01em] text-neutral-950 group-hover:text-[#2563EB]"
    ...
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
      {Array.from({ length: 6 }).map((_, i) => (
        ...
        >
          ...
          <div className="mt-1.5 h-4 w-5/6 animate-pulse rounded bg-[#f0f0f0]" />
